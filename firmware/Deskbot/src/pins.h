...
// Audio has no wall-clock catch-up/resync mechanism -- audioTaskFn just
// decodes whatever MP3 bytes arrive, in order. A previous attempt at one
// (skip-ahead by discarding undecoded bytes) desynced the Helix decoder's
// bitstream parser and made audio sound worse (cracking/wrong-pitched)
// than the lip-sync drift it was meant to fix, so it was removed.
